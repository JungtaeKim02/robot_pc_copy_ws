...
// [v2 디버깅] 후보별 진단 메시지
#include "inha_interfaces/msg/candidate_result.hpp"
#include "inha_interfaces/msg/request_summary.hpp"
...
using CandidateResult = inha_interfaces::msg::CandidateResult;
using RequestSummary = inha_interfaces::msg::RequestSummary;
...
public:
  ...
  : Node("rby1_grasp_action_server", options)
  {
    ...
    // [v2 디버깅] RequestSummary publisher (한 요청당 1회 발행)
    summary_pub_ = this->create_publisher<RequestSummary>(
      "/manipulation/debug/request_summary", rclcpp::QoS(10).reliable().transient_local());

    // [v2 디버깅] 색상 마커 publisher (Foxglove 3D 시각화용)
    candidate_markers_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
      "/manipulation/debug/candidate_markers", rclcpp::QoS(10).reliable().transient_local());
    ...
    RCLCPP_INFO(get_logger(),
      "🚀Rby1 Grasp Action Server [v2] Created. Waiting for MoveIt...\n"
      "   Improvements:\n"
      "   ① approach 경로 collision 사전 필터 (octomap-based EEF path check)\n"
      "   ② real-grasp IK → seed → pre-grasp IK (joint-space 일관성 확보)");
  }
...
private:
  ...
  // [v2 디버깅] 진단 publisher
  rclcpp::Publisher<RequestSummary>::SharedPtr summary_pub_;
  ...
  // ==========================================
  // [개선 1] approach 경로 collision pre-filter
  //
  // pre-grasp pose → real-grasp pose 사이의 EEF 직선 경로상에
  // octomap occupancy 가 있는지 EEF 표면 샘플 단위로 확인한다.
  //
  // - 이는 "필요조건" 만 검사한다 (관절 충돌은 별개).
  //   IK 와 computeCartesianPath 비용을 들이기 전에 명백히 막힌
  //   후보를 빠르게 걸러내는 빠른 사전 필터.
  // - real-grasp pose 자체는 물체와 overlap 되어있으므로
  //   real-grasp 위치(t=1.0)는 검사 대상에서 제외한다.
  //   = 검사 구간 t ∈ [0.0, t_max]  with t_max < 1.0
  //
  // 반환:
  //   true  : 경로가 비어있어 보임 (다음 IK 단계로 진행)
  //   false : 막혀있음 (이 후보 스킵)
  // ==========================================
  bool isApproachPathClear(
      const geometry_msgs::msg::Pose& pregrasp_world,
      const geometry_msgs::msg::Pose& realgrasp_world,
      const planning_scene::PlanningScenePtr& scene,
      std::string& detail_out)
  {
    detail_out.clear();
    if (!scene) return true;  // scene 없으면 필터 비활성 (안전 우선)
    ...
    if (!octomap_obj || octomap_obj->shapes_.empty()) {
      RCLCPP_WARN_ONCE(get_logger(),
        "[ApproachPath] No octomap in planning scene, skipping pre-filter.");
      return true;
    }
    auto octree_shape = std::dynamic_pointer_cast<const shapes::OcTree>(
        octomap_obj->shapes_[0]);
    if (!octree_shape || !octree_shape->octree) {
      RCLCPP_WARN_ONCE(get_logger(), "[ApproachPath] OcTree shape cast failed.");
      return true;
    }
    const auto& octree = octree_shape->octree;
    Eigen::Isometry3d octomap_tf = Eigen::Isometry3d::Identity();
    ...
    // ---- 두 끝점 EE world transform 구성 ----
    auto build_T = [](const geometry_msgs::msg::Pose& p) {
    ...
    Eigen::Isometry3d T_pre  = build_T(pregrasp_world);
    Eigen::Isometry3d T_real = build_T(realgrasp_world);

    // ---- 경로상의 sample 포즈 생성 (선형 보간) ----
    // t=0 : pregrasp,  t=1 : realgrasp (검사 제외)
    // realgrasp 자체는 물체와 overlap 되므로 t_max=0.6 까지만 검사.
    // (그 너머는 그리퍼-물체 접촉 영역, 물체 collision 도 어차피 무시 대상)
    constexpr int N_PATH_SAMPLES = 6;
    constexpr double T_MAX = 0.6;

    // ---- EE local 표면 샘플 (그리퍼 finger tip / arm_5 cylinder)
    //   approach 경로상의 EEF 자세에서, EE 주변 표면이 octomap 과 겹치는지 검사.
    //   세부 샘플은 checkRigidBlockCollision 과 동일 철학.
    struct SamplePt { double x, y, z; const char* label; };
    static const SamplePt samples[] = {
      // finger tips & mid (grasp 방향 앞단)
      { 0.05,  0.0, -0.135, "finger1_tip"},
      {-0.05,  0.0, -0.135, "finger2_tip"},
      { 0.05,  0.0, -0.105, "finger1_mid"},
      {-0.05,  0.0, -0.105, "finger2_mid"},
    ...
    for (int s = 0; s <= N_PATH_SAMPLES; ++s) {
      double t = T_MAX * static_cast<double>(s) / N_PATH_SAMPLES;

      // pose 보간 — position 은 선형, rotation 은 slerp
      Eigen::Vector3d pos_interp =
          (1.0 - t) * T_pre.translation() + t * T_real.translation();
      Eigen::Quaterniond q_pre(T_pre.rotation());
      Eigen::Quaterniond q_real(T_real.rotation());
      Eigen::Quaterniond q_interp = q_pre.slerp(t, q_real);

      Eigen::Isometry3d T_ee = Eigen::Isometry3d::Identity();
      T_ee.translation() = pos_interp;
      T_ee.linear() = q_interp.toRotationMatrix();

      for (size_t k = 0; k < N_SAMPLES; ++k) {
        Eigen::Vector3d pt_local(samples[k].x, samples[k].y, samples[k].z);
        Eigen::Vector3d pt_world = T_ee * pt_local;
        Eigen::Vector3d pt_oct   = octomap_tf_inv * pt_world;
        auto node = octree->search(pt_oct.x(), pt_oct.y(), pt_oct.z());
        if (node && octree->isNodeOccupied(node)) {
          ...
          if (first_hit.empty()) {
            char buf[64];
            snprintf(buf, sizeof(buf), "t=%.2f/%s", t, samples[k].label);
            first_hit = buf;
          }
        }
      }
    ...
    if (n_hits > 0) {
      char buf[256];
      snprintf(buf, sizeof(buf), "%zu hits, first=%s", n_hits, first_hit.c_str());
      detail_out = buf;
      return false;
    }
    return true;
  ...
  // ==========================================
  // [v2 디버깅] CandidateResult 초기화 헬퍼
  //   각 후보의 진단 결과를 NaN/기본값으로 초기화한다.
  // ==========================================
  static void initCandidateResult(
      CandidateResult& cr,
      uint32_t idx,
      ...
      const geometry_msgs::msg::Pose& realgrasp_world)
  {
    cr.candidate_index = idx;
    cr.variant_index = 0;
    cr.source_pose = source_pose;
    cr.pregrasp_world = pregrasp_world;
    cr.realgrasp_world = realgrasp_world;
    cr.final_stage = "not_attempted";
    cr.failure_reason = "";
    constexpr double NaN = std::numeric_limits<double>::quiet_NaN();
    cr.manipulability = NaN;
    cr.ns_cost = NaN;
    cr.step2_coverage_percent = NaN;
    cr.step2_trim_distance_m = NaN;
    cr.step2_direction_alignment = NaN;
    cr.q_realgrasp_pregrasp_distance_rad = NaN;
    cr.elapsed_ms = NaN;
  }

  // ==========================================
  // [v2 디버깅] 후보 final_stage 업데이트 헬퍼
  // ==========================================
  static void setCandidateStage(
      CandidateResult& cr,
      const std::string& stage,
      const std::string& reason)
  {
    cr.final_stage = stage;
    cr.failure_reason = reason;
  }

  // ==========================================
  // [v2 디버깅] final_stage → 색상 매핑
  //   Foxglove 3D 패널에서 후보별 색상으로 결과를 시각화.
  // ==========================================
  static std::array<float, 4> stageToColor(const std::string& stage)
  {
    // {r, g, b, a}
    if (stage == "winner_executed")              return {0.0f, 1.0f, 0.0f, 1.0f}; // green
    if (stage == "step2_low_coverage" ||
        stage == "step2_misaligned" ||
        stage == "step2_exec_failed")            return {1.0f, 0.5f, 0.0f, 1.0f}; // orange
    if (stage == "step1_plan_failed" ||
        stage == "step1_exec_failed")            return {1.0f, 1.0f, 0.0f, 1.0f}; // yellow
    if (stage == "pregrasp_ik_failed" ||
        stage == "realgrasp_ik_failed" ||
        stage == "ik_imprecise")                 return {1.0f, 0.0f, 0.0f, 1.0f}; // red
    if (stage == "approach_path_blocked")        return {0.5f, 0.5f, 0.5f, 1.0f}; // gray
    if (stage == "not_attempted")                return {0.3f, 0.3f, 0.3f, 0.5f}; // dark gray
    return {0.0f, 0.5f, 1.0f, 1.0f};                                              // blue (unknown)
  }

  // ==========================================
  // [v2 디버깅] RequestSummary + MarkerArray 발행
  //
  //   handle_service 종료 시점에 호출.
  //   per_candidate_results 와 GraspLog 의 카운트 정보를 합쳐
  //   RequestSummary 를 만들고, 후보별 색상 마커도 함께 발행.
  // ==========================================
  void publishDiagnostics(
      const std::string& request_id,
      const std::string& arm_id,
      const std::string& mode,
      const std::vector<CandidateResult>& per_candidate_results,
      const std::string& planning_frame,
      bool overall_success,
      int32_t winning_index,
      uint32_t winning_batch,
      uint32_t winning_rank,
      const std::string& final_status_message,
      double total_elapsed_ms,
      uint32_t n_input,
      uint32_t n_after_approach_filter,
      uint32_t n_realgrasp_ik_ok,
      uint32_t n_pregrasp_ik_ok,
      uint32_t n_step1_plan_ok,
      uint32_t n_step2_cartesian_ok)
  {
    // ----- RequestSummary -----
    RequestSummary summary;
    summary.header.stamp = this->get_clock()->now();
    summary.header.frame_id = planning_frame;
    summary.request_id = request_id;
    summary.arm_id = arm_id;
    summary.mode = mode;
    summary.pose_source = "";  // action_server 단계에서는 알 수 없음. manager 가 보강.

    summary.overall_success = overall_success;
    summary.winning_candidate_index = winning_index;
    summary.winning_variant_index = -1;  // prefilter 정보 없으면 미상
    summary.winning_batch_index = winning_batch;
    summary.winning_rank = winning_rank;
    summary.final_status_message = final_status_message;

    summary.n_input = n_input;
    summary.n_after_manager_reach = n_input;     // manager 정보 없으면 = n_input
    summary.n_after_prefilter = n_input;         // prefilter 정보 없으면 = n_input
    summary.n_after_approach_filter = n_after_approach_filter;
    summary.n_realgrasp_ik_ok = n_realgrasp_ik_ok;
    summary.n_pregrasp_ik_ok = n_pregrasp_ik_ok;
    summary.n_step1_plan_ok = n_step1_plan_ok;
    summary.n_step2_cartesian_ok = n_step2_cartesian_ok;

    summary.candidates = per_candidate_results;
    ...
    // 기존 마커 삭제용
    {
      ...
      del.header.frame_id = planning_frame;
      del.header.stamp = summary.header.stamp;
      del.ns = "grasp_candidates_v2";
      del.action = visualization_msgs::msg::Marker::DELETEALL;
      marker_array.markers.push_back(del);
    }

    // 후보별 화살표 (EE 로컬 -Z 방향 = approach 방향)
    int marker_id = 0;
    for (const auto& cr : per_candidate_results) {
      auto color = stageToColor(cr.final_stage);

      visualization_msgs::msg::Marker m;
      m.header.frame_id = planning_frame;
      m.header.stamp = summary.header.stamp;
      m.ns = "grasp_candidates_v2";
      m.id = marker_id++;
      m.type = visualization_msgs::msg::Marker::ARROW;
      m.action = visualization_msgs::msg::Marker::ADD;
      m.pose = cr.realgrasp_world;
      // 화살표 크기: 길이 6cm, 두께 1cm
      m.scale.x = 0.06;
      m.scale.y = 0.01;
      m.scale.z = 0.01;
      m.color.r = color[0];
      ...
      m.color.b = color[2];
      m.color.a = color[3];
      m.lifetime = rclcpp::Duration::from_seconds(0);  // 영구
      marker_array.markers.push_back(m);

      // 후보 인덱스 텍스트
      visualization_msgs::msg::Marker t;
      t.header = m.header;
      t.ns = "grasp_candidates_v2_text";
      t.id = marker_id++;
      t.type = visualization_msgs::msg::Marker::TEXT_VIEW_FACING;
      t.action = visualization_msgs::msg::Marker::ADD;
      t.pose.position = cr.realgrasp_world.position;
      t.pose.position.z += 0.03;  // 위쪽에 표시
      t.pose.orientation.w = 1.0;
      t.scale.z = 0.02;
      t.color.r = 1.0; t.color.g = 1.0; t.color.b = 1.0; t.color.a = 1.0;
      t.text = std::to_string(cr.candidate_index);
      t.lifetime = m.lifetime;
      marker_array.markers.push_back(t);
    }

    candidate_markers_pub_->publish(marker_array);

    RCLCPP_INFO(get_logger(),
      "📊 [Diagnostic] Published RequestSummary (request_id=%s) and %zu candidate markers.",
      request_id.c_str(), per_candidate_results.size());
  }
    ...
    std::shared_ptr<ExecuteGraspSrv::Response> response)
  {
    ...
    // ==========================================
    // [v2 디버깅] request_id 결정 — 빈 문자열이면 노드에서 새로 생성
    std::string request_id = request->request_id;
    if (request_id.empty()) {
      // UUID 대용으로 epoch ns + arm_id 사용 (외부에서 주입 안 됐을 때 fallback)
      auto ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
          std::chrono::system_clock::now().time_since_epoch()).count();
      request_id = std::to_string(ns) + "_" + request->arm_id;
    }

    // [v2 디버깅] 후보별 진단 결과 컨테이너 (입력 후보 갯수 만큼)
    std::vector<CandidateResult> per_candidate_results(total);

    // 진단 카운터 (단계별 통과 수)
    uint32_t n_after_approach_filter = 0;
    uint32_t n_realgrasp_ik_ok = 0;
    uint32_t n_pregrasp_ik_ok = 0;
    uint32_t n_step1_plan_ok = 0;
    uint32_t n_step2_cartesian_ok = 0;

    // [개선] world_poses 외에 realgrasp_world_poses 도 함께 저장.
    //   world_poses[i]        = pre-grasp world pose (standoff 적용)
    //   realgrasp_world_poses[i] = real-grasp world pose (입력 그대로, standoff 없음)
    //
    //   - 개선 1 (approach 경로 collision 사전 필터) 에서 두 pose 가 모두 필요
    //   - 개선 2 (real-grasp IK → seed → pre-grasp IK) 에서 real-grasp pose 가 필요
    std::vector<geometry_msgs::msg::Pose> world_poses;
    std::vector<geometry_msgs::msg::Pose> realgrasp_world_poses;
    world_poses.reserve(total);
    realgrasp_world_poses.reserve(total);
    {
      ...
      auto to_msg = [](const Eigen::Isometry3d& T) {
      ...
      for (const auto& p : poses) {
        ...
        Eigen::Isometry3d p_w_real = need_transform ? (frame_tf * p_in) : p_in;

        // [real-grasp world pose] standoff 적용 전 — 실제 잡는 자세
        realgrasp_world_poses.push_back(to_msg(p_w_real));

        // [pre-grasp world pose] standoff 적용 — EE 로컬 +Z 로 뒤로 물림
        // approach_dir = -local_z 이므로 +local_z 는 "후퇴" 방향.
        Eigen::Isometry3d p_w_pre = p_w_real;
        Eigen::Vector3d plus_z_world = p_w_pre.rotation().col(2);
        p_w_pre.translation() += plus_z_world * PREGRASP_STANDOFF;
        world_poses.push_back(to_msg(p_w_pre));
      }
      RCLCPP_INFO(get_logger(),
        "[Frame] Pre-computed %zu world poses (transform=%s, both pre-grasp + real-grasp).",
        world_poses.size(), need_transform ? "yes" : "identity");

      // [v2 디버깅] per_candidate_results 초기화 (모든 후보에 대해)
      for (size_t i = 0; i < total; ++i) {
        initCandidateResult(per_candidate_results[i],
                            static_cast<uint32_t>(i),
                            poses[i],                   // source_pose (input frame)
                            world_poses[i],             // pre-grasp world
                            realgrasp_world_poses[i]);  // real-grasp world
      }
    }
    ...
    // [개선 2] real-grasp IK 용 별도 validity callback
    //
    //   real-grasp pose 는 정의상 그리퍼가 물체와 overlap 된 상태이므로,
    //   pre-grasp 와 동일한 collision 검사를 적용하면 거의 모든 해가 reject 된다.
    //   이 callback 은 torso ±15° 제약만 적용하고 collision 검사는 생략한다.
    //   (real-grasp 해는 단지 pre-grasp IK 의 seed 로 사용되며, 최종적으로
    //    pre-grasp IK 가 통과해야만 채택되므로 이 단계의 collision 미검사는 안전)
    moveit::core::GroupStateValidityCallbackFn validity_callback_realgrasp;
    validity_callback_realgrasp = [&torso_joint_names, &current_torso_vals,
                                    TORSO_MAX_DELTA_RAD](
        moveit::core::RobotState* state,
        const moveit::core::JointModelGroup* group,
        const double* joint_values) -> bool {
      ...
      for (size_t tj = 0; tj < torso_joint_names.size(); ++tj) {
        ...
        if (!jm) continue;
        double delta = std::abs(state->getJointPositions(jm)[0] - current_torso_vals[tj]);
        if (delta > TORSO_MAX_DELTA_RAD) return false;
      }
      return true;
    };
    ...
    for (size_t batch_start = 0; batch_start < total; batch_start += BATCH_SIZE) {
      ...
      for (size_t i = batch_start; i < batch_end; ++i) {
        ...
        //   pre-grasp → real-grasp 사이 EEF 직선 경로상에 octomap 점이
        //   있는지 확인. 명백히 막힌 후보는 IK 단계 진입 전에 스킵.
        //   (place 모드에서는 standoff/approach 가 없으므로 스킵)
        // ==========================================
        if (current_scene && !is_place_mode) {
          std::string ap_detail;
          bool path_clear = isApproachPathClear(
              target_pose_world,           // pre-grasp
              realgrasp_world_poses[i],    // real-grasp
              current_scene,
              ap_detail);
          if (!path_clear) {
            RCLCPP_WARN(get_logger(),
              "🚫 [Pre-Filter] Cand %zu: approach path BLOCKED (%s). Skip IK.",
              i + 1, ap_detail.c_str());
            batch_log.ik_results.push_back(
                {i + 1, true, "approach_path_blocked: " + ap_detail, 0, 0.0, 0.0, 0.0});
            // [v2 디버깅]
            setCandidateStage(per_candidate_results[i],
                              "approach_path_blocked", ap_detail);
            continue;
          }
          n_after_approach_filter++;
          RCLCPP_INFO(get_logger(),
            "✅ [Pre-Filter] Cand %zu: approach path clear, proceed to IK.", i + 1);
        } else {
          // pre-filter 비활성화 (place 모드 등) → 통과로 간주
          n_after_approach_filter++;
        }
        ...
        // [개선 2] real-grasp pose 도 함께 준비
        const geometry_msgs::msg::Pose& realgrasp_pose_world = realgrasp_world_poses[i];
        ...
        // ==========================================
        // [개선 2] IK 시도 루프 — real-grasp IK → seed → pre-grasp IK
        //
        //   순서:
        //     ① 현재 시도 seed 결정 (현재자세 / 랜덤)
        //     ② real-grasp pose 로 IK 시도 (그리퍼-물체 overlap 허용)
        //         - 실패 → 이 후보는 물리적으로 잡기 어려움 → 다음 attempt
        //     ③ ② 의 해 (q_realgrasp) 를 seed 로 pre-grasp pose IK
        //         - q_pregrasp ≈ q_realgrasp (joint space 에서 가까운 해)
        //         - Cartesian Step2 가 구조적으로 안정적
        //     ④ pre-grasp 해의 정밀도 / null-space cost 평가 (기존 그대로)
        //
        //   real-grasp IK 추가 진단 카운터
        size_t n_realgrasp_fail = 0;
        // ==========================================
        for (size_t attempt = 0; attempt < N_IK_ATTEMPTS; ++attempt) {
          ...
          // ===================================================================
          // [개선 2 - ②] real-grasp pose 로 IK
          //   - validity_callback_realgrasp 사용: torso ±15° 만 검사,
          //     그리퍼-물체 collision 은 무시 (정의상 overlap 상태이므로)
          //   - place 모드에서는 standoff=0 이므로 pre-grasp = real-grasp 동일.
          //     이 경우 별도 real-grasp IK 단계 없이 바로 ③ 으로 진행.
          // ===================================================================
          auto realgrasp_state = std::make_shared<moveit::core::RobotState>(*try_state);
          bool found_rg = true;
          double rg_ms = 0.0;

          if (!is_place_mode) {
            auto t_rg0 = std::chrono::steady_clock::now();
            found_rg = realgrasp_state->setFromIK(
                jmg_ik, realgrasp_pose_world, target_ee_link,
                IK_TIMEOUT, validity_callback_realgrasp, ik_opts);
            rg_ms = std::chrono::duration<double, std::milli>(
                std::chrono::steady_clock::now() - t_rg0).count();
          }
          ...
          if (!found_rg) {
            ...
            if (detailed_log && attempt < N_DETAILED_ATT) {
              RCLCPP_INFO(get_logger(),
                "  [IK att %zu] Cand %zu: real-grasp IK FAIL (%.1f ms, seed=%s)",
                attempt, i + 1, rg_ms, (attempt < 2 ? "current" : "random"));
            }
            // real-grasp IK 가 안 풀린다 = 이 후보 자세 자체가 도달 불가
            // 다음 attempt 의 다른 seed 로 재시도
            consecutive_fail++;
            if (consecutive_fail >= EARLY_ABORT_FAILS && n_found == 0) {
              RCLCPP_WARN(get_logger(),
                "  [IK] Cand %zu: %zu consecutive realgrasp-IK FAIL (dist=%.3fm) "
          ...
          // ===================================================================
          // [개선 2 - ③] q_realgrasp 를 seed 로 pre-grasp IK
          //   try_state 의 jmg_ik 관절을 q_realgrasp 로 초기화 → setFromIK
          //   TRAC-IK 가 이 seed 근방에서 pre-grasp 해를 탐색
          //   → q_pregrasp ≈ q_realgrasp (joint space 에서 가까운 해)
          // ===================================================================
          if (!is_place_mode) {
            std::vector<double> q_realgrasp;
            realgrasp_state->copyJointGroupPositions(jmg_ik, q_realgrasp);
            try_state->setJointGroupPositions(jmg_ik, q_realgrasp);
          } else {
            // place 모드: try_state 가 이미 적절한 seed (현재자세/랜덤) 로 설정됨.
            // realgrasp_pose_world == target_pose_world 이므로 별도 처리 불필요.
          }
          ...
          if (!found) {
            if (detailed_log && attempt < N_DETAILED_ATT) {
              RCLCPP_INFO(get_logger(),
                "  [IK att %zu] Cand %zu: pre-grasp IK FAIL (%.1f ms, rg=%.1fms, seed=%s)",
                attempt, i + 1, ik_ms, rg_ms, (attempt < 2 ? "current" : "random"));
            }
            consecutive_fail++;
            if (consecutive_fail >= EARLY_ABORT_FAILS && n_found == 0) {
              RCLCPP_WARN(get_logger(),
        ...
        RCLCPP_INFO(get_logger(),
          "[IK Debug] Cand %zu: valid=%zu/%zu  precise=%zu  rg_fail=%zu  "
          "cb_reject(torso=%zu, collision=%zu)  min_pos=%.4f m  min_orient=%.1f°",
          i + 1, n_found, N_IK_ATTEMPTS, n_precise, n_realgrasp_fail,
        ...
        if (!best_state) {
          ...
          // [v2 디버깅] IK 단계 실패 분류
          //   real-grasp IK 가 한 번도 안 풀렸다면 realgrasp_ik_failed
          //   real-grasp 는 풀렸지만 pre-grasp 가 모두 실패한 경우 pregrasp_ik_failed
          //   real-grasp 는 풀렸지만 정밀도 미달이면 ik_imprecise
          if (n_realgrasp_fail >= N_IK_ATTEMPTS) {
            setCandidateStage(per_candidate_results[i],
                              "realgrasp_ik_failed",
                              "all " + std::to_string(N_IK_ATTEMPTS)
                              + " real-grasp IK attempts failed");
          } else if (n_found == 0) {
            setCandidateStage(per_candidate_results[i],
                              "pregrasp_ik_failed",
                              "pre-grasp IK never converged after real-grasp seed");
          } else {
            setCandidateStage(per_candidate_results[i],
                              "ik_imprecise",
                              "IK found but pos/orient error exceeded threshold");
          }
          continue;
        }
        n_realgrasp_ik_ok++;
        ...
        // [v2 디버깅] IK 통과한 후보의 정량 지표 채움
        per_candidate_results[i].manipulability = best_manip;
        per_candidate_results[i].ns_cost = best_ns_cost;
      ...
      for (size_t rank = 0; rank < n_plan_candidates; ++rank) {
        ...
        if (step1_result != moveit::core::MoveItErrorCode::SUCCESS) {
          ...
          // [v2 디버깅]
          setCandidateStage(per_candidate_results[cand.original_index],
                            "step1_plan_failed",
                            "Step1 plan failed: " + errorCodeToString(step1_result.val));
          continue;
        }
        ...
        if (is_place_mode) {
          ...
          // [v2 디버깅] place 성공 진단 발행
          setCandidateStage(per_candidate_results[cand.original_index],
                            "winner_executed",
                            "[PLACE] Selected as winner and placed.");
          publishDiagnostics(
            request_id, request->arm_id, mode,
            per_candidate_results, planning_frame,
            true, static_cast<int32_t>(cand.original_index),
            static_cast<uint32_t>(batch_log.batch_index),
        ...
        // [v2 디버깅] Step2 coverage 기록
        per_candidate_results[cand.original_index].step2_coverage_percent = fraction * 100.0;
        ...
        if (fraction < min_required_fraction) {
          ...
          setCandidateStage(per_candidate_results[cand.original_index],
                            "step2_low_coverage",
                            "Cartesian coverage " + std::to_string(fraction * 100.0)
                            + "% < required " + std::to_string(min_required_fraction * 100.0) + "%");
          continue;
        }
        ...
        if (cartesian_trajectory.joint_trajectory.points.size() < 2) {
          ...
          setCandidateStage(per_candidate_results[cand.original_index],
                            "step2_low_coverage",
                            "Cartesian trajectory has fewer than 2 points");
          continue;
        }
        ...
        if (!trim_valid) {
          ...
          // [v2 디버깅]
          per_candidate_results[cand.original_index].step2_trim_distance_m = actual_distance;
          setCandidateStage(per_candidate_results[cand.original_index],
                            "step2_low_coverage",
                            "Trim distance " + std::to_string(actual_distance)
                            + "m out of [" + std::to_string(MIN_APPROACH_DISTANCE)
                            + "," + std::to_string(MAX_APPROACH_DISTANCE) + "] range");
          continue;
        }
        per_candidate_results[cand.original_index].step2_trim_distance_m = actual_distance;
        ...
        // 이동 방향 검증: 실제 이동이 의도한 EEF -Z 방향과 일치하는지
        {
          ...
          if (actual_move_len > 1e-6) {
            ...
            RCLCPP_INFO(get_logger(), "[Step2 Check] Direction alignment: %.3f (intended vs actual, 1.0=perfect)",
                        alignment);
            // [v2 디버깅]
            per_candidate_results[cand.original_index].step2_direction_alignment = alignment;
            if (alignment < 0.5) {
              ...
              setCandidateStage(per_candidate_results[cand.original_index],
                                "step2_misaligned",
                                "Direction alignment " + std::to_string(alignment) + " < 0.5");
              continue;
            }
          }
        }
        ...
        // [v2 디버깅] winner 표시 + 진단 발행
        setCandidateStage(per_candidate_results[cand.original_index],
                          "winner_executed",
                          "Selected as final winner and executed.");
        publishDiagnostics(
          request_id, request->arm_id, mode,
          per_candidate_results, planning_frame,
          true, static_cast<int32_t>(cand.original_index),

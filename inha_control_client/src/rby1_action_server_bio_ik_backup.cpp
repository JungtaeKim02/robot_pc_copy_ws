...
public:
  ...
  void init_moveit()
  {
    ...
    try {
        ...
        move_group_right_torso_->setMaxVelocityScalingFactor(0.06);
        move_group_right_torso_->setMaxAccelerationScalingFactor(0.06);
    ...
    try {
        ...
        move_group_left_torso_->setMaxVelocityScalingFactor(0.05);
        move_group_left_torso_->setMaxAccelerationScalingFactor(0.05);
...
private:
    ...
    std::shared_ptr<PlaybackGrasp::Response> response)
  {
    ...
    const double LIFT_HEIGHT = 0.07;  // 7cm
    const int N_LIFT_WAYPOINTS = 10;   // 명시적 중간 경유지 개수 (직선성 보장)
    ...
    RCLCPP_INFO(get_logger(), "[Lift] 실제 실행을 생략하고 플래닝 성공으로 처리합니다.");
    ...
    std::shared_ptr<ExecuteGraspSrv::Response> response)
  {
    ...
    const double PREGRASP_STANDOFF      = 0.08;   // 8 cm: 물체 voxel 여유
    const double MIN_APPROACH_DISTANCE  = 0.06;   // Step 2 trim 하한 (6 cm)
    const double MAX_APPROACH_DISTANCE  = 0.09;   // Step 2 trim 상한 (9 cm)
    const double LONG_APPROACH_DISTANCE = 0.10;   // Step 2 plan sweep 길이 (10 cm)
    const double STEP1_SPEED_SCALE      = 0.3;    // Step 1 (pregrasp 이동) 속도 스케일
    ...
    //
    //      [Antipodal yaw flip] 평행-조 그리퍼는 approach(local Z) 축
    //      180° 회전에 대해 물리적으로 동일한 파지이다. 이 공짜 자유도를
    //      이용해 "손목 카메라 링크가 바닥(월드 -Z)을 바라보는" orientation
    //      을 회피한다. 카메라는 오른팔 EE 로컬 -Y, 왼팔 EE 로컬 +Y 에
    //      배치되어 있으므로, (R_eef * camera_local).z() < 0 이면
    //      카메라가 아래쪽을 향하는 상태이다. 이 경우 EE 로컬 Z 주위로
    //      180° 회전시키면 잡는 형태는 같고 카메라만 위로 올라간다.
    //      (전략 B: 더 나은 쪽 1개만 유지)
    // ==========================================
    const Eigen::Vector3d camera_local_offset =
        (request->arm_id == "right") ? Eigen::Vector3d(0.0, -1.0, 0.0)
                                     : Eigen::Vector3d(0.0, +1.0, 0.0);
    const Eigen::Matrix3d R_yaw180 =
    ...
    size_t yaw_flip_count = 0;
    {
      ...
      for (const auto& p : poses) {
        ...
        // ---- Antipodal yaw flip (카메라가 바닥을 바라보는 경우 회피) ----
        // camera_dir_world = R_eef_world * camera_local_offset
        // z 성분이 음수면 카메라가 world -Z(바닥) 쪽을 향함 → 180° 뒤집기
        Eigen::Vector3d camera_dir_world = p_w.rotation() * camera_local_offset;
        if (camera_dir_world.z() < 0.0) {
          // EE 로컬 Z축 기준 180° 회전 (오른쪽에서 곱해야 로컬축 회전)
          p_w.linear() = p_w.rotation() * R_yaw180;
          yaw_flip_count++;
        }

        // ---- Pregrasp standoff: grasp pose 에서 EE 로컬 +Z 로 뒤로 물림 ----
        // approach_dir = -local_z 이므로 +local_z 는 "후퇴" 방향.
        // world frame 으로 표현: R_eef * Z_local = p_w.rotation().col(2)
      ...
      RCLCPP_INFO(get_logger(),
        "[CameraAvoid] Antipodal yaw-flip applied to %zu/%zu candidates "
        "(camera_local=%s-Y, rule: camera_dir_world.z < 0 → rotate 180° around EE local Z).",
        yaw_flip_count, world_poses.size(),
        (request->arm_id == "right") ? "-" : "+");
    }
    ...
    // ==========================================
    // 현재 torso 관절 값 캡처 (null-space cost 에서 torso 변화량 페널티용)
    const std::vector<std::string> ns_torso_names = {"torso_3", "torso_4", "torso_5"};
    std::vector<double> ns_current_torso(ns_torso_names.size(), 0.0);
    {
      auto cur_st = active_group_torso->getCurrentState();
      for (size_t tj = 0; tj < ns_torso_names.size(); ++tj) {
        const auto* jm = cur_st->getJointModel(ns_torso_names[tj]);
        if (jm) ns_current_torso[tj] = cur_st->getJointPositions(jm)[0];
      }
    }

    const auto compute_nullspace_cost = [this, &target_ee_link, &ns_torso_names, &ns_current_torso](
      ...
      for (size_t tj = 0; tj < ns_torso_names.size(); ++tj) {
        const auto* jm = state.getJointModel(ns_torso_names[tj]);
        if (!jm) continue;
        double delta = state.getJointPositions(jm)[0] - ns_current_torso[tj];
    ...
    for (size_t batch_start = 0; batch_start < total; batch_start += BATCH_SIZE) {
      ...
      for (size_t i = batch_start; i < batch_end; ++i) {
        ...
        RCLCPP_INFO(get_logger(),
          ...
          "dist_from_shoulder=%.3f m  (R_MAX=0.85)",
          i + 1,
        ...
        // 도달 반경 안에 있어야 말이 됨 — 아니면 강력 경고
        if (dist_from_shoulder > 0.85) {
          RCLCPP_WARN(get_logger(),
            "[IK] Cand %zu: target is BEYOND R_MAX (%.3f > 0.85) — IK is unlikely to converge.",
            i + 1, dist_from_shoulder);
        }

        // ---- Rigid-block pre-filter ----
        // IK 해와 무관한 고정 링크(arm_5~fingers) 블록이 octomap 과 충돌하면
        // IK 시도 없이 즉시 skip → 연산 시간 대폭 절감
        {
          std::string rb_detail;
          bool rb_collision = checkRigidBlockCollision(
              target_pose_world, current_scene, i + 1, rb_detail);

          // IK 로그 엔트리에 rigid-block 결과 기록
          IkLogEntry ik_log_entry;
          ik_log_entry.candidate_index = i + 1;
          ik_log_entry.rigid_block_skip = rb_collision;
          ik_log_entry.rigid_block_detail = rb_detail;
          ik_log_entry.precise_solutions = 0;
          ik_log_entry.best_ns_cost = 0;
          ik_log_entry.best_manip = 0;
          ik_log_entry.best_joint = 0;

          if (rb_collision) {
            RCLCPP_WARN(get_logger(),
              "[IK] Cand %zu: SKIPPED by rigid-block pre-filter — %s",
              i + 1, rb_detail.c_str());
            batch_log.ik_results.push_back(ik_log_entry);
            continue;  // 다음 후보로
          }
          // 통과 시 ik_log_entry 는 아래에서 IK 결과로 덮어씀
        }
        ...
        // IK를 N회 시도.
        // TRAC-IK Distance mode 는 approximate 해를 반환하지 않으므로 found=true 는 대체로 정밀.
        // 다만 MoveIt 인터페이스 일관성을 위해 approximate 플래그는 유지하고 사후 필터로 확인.
        ...
        // 현재 torso 관절 값 캡처 (IK 해의 torso 변화량 제한용)
        ...
        for (size_t tj = 0; tj < torso_joint_names.size(); ++tj) {
          const auto* jm = ref_state->getJointModel(torso_joint_names[tj]);
          if (jm) current_torso_vals[tj] = ref_state->getJointPositions(jm)[0];
        }

        // 진단 카운터
        size_t n_found        = 0;   // setFromIK 가 true 를 반환
        size_t n_kin_ok       = 0;   // pos<1cm, orient<10°
        size_t n_collision    = 0;   // kin_ok 해 중 scene collision
        size_t n_self_col     = 0;
        size_t n_env_col      = 0;
        size_t n_torso_reject = 0;   // torso 변화량 초과로 거부
        double min_pos_err        = 1e9;
        double min_orient_err_deg = 1e9;

        // 최초 2건의 충돌 pair 샘플
        std::vector<std::string> first_collision_samples;
        ...
        for (size_t attempt = 0; attempt < N_IK_ATTEMPTS; ++attempt) {
          ...
          //  - attempt >= 2 : arm JMG 만 randomize (torso 는 Phase 1 이후 고정)
          if (attempt >= 2) {
          ...
          bool found = try_state->setFromIK(
              ...
              IK_TIMEOUT, nullptr, ik_opts);  // validity=nullptr: 순수 기구학 검증
          ...
          if (detailed_log && attempt < N_DETAILED_ATT) {
            RCLCPP_INFO(get_logger(),
              "  [IK att %zu] Cand %zu: found (%.1f ms, seed=%s) "
          ...
          if (current_scene) {
            ...
            creq.group_name = jmg_ik->getName();

            collision_detection::CollisionResult cres;
            current_scene->checkCollision(creq, cres, *try_state);

            if (cres.collision) {
              n_collision++;
              bool is_self = false, is_env = false;

              for (const auto& kv : cres.contacts) {
                const std::string& a = kv.first.first;
                const std::string& b = kv.first.second;

                // "<octomap>" / attached object 등은 robot model 에 등록되지 않은
                // 가상 엔티티이므로 getLinkModel 을 호출하면 ERROR 로그 스팸.
                // 이름이 "<...>" 로 감싸여 있거나 robot model 에 없는 이름은 env 로 간주.
                auto is_robot_link = [this, &ref_state](const std::string& name) -> bool {
                  if (!name.empty() && name.front() == '<') return false;
                  return ref_state->getRobotModel()->hasLinkModel(name);
                };
                bool a_is_robot = is_robot_link(a);
                bool b_is_robot = is_robot_link(b);
                if (a_is_robot && b_is_robot) is_self = true;
                else                           is_env  = true;

                if (first_collision_samples.size() < 2) {
                  first_collision_samples.push_back(a + " <-> " + b);
                }
              }

              if (is_self) n_self_col++;
              if (is_env)  n_env_col++;

              if (detailed_log && attempt < N_DETAILED_ATT) {
                RCLCPP_INFO(get_logger(),
                  "    -> collision (self=%d, env=%d)", (int)is_self, (int)is_env);
              }
              continue;  // 충돌 해는 스킵
            }
          }

          // Torso 변화량 제한: 현재 torso 에서 ±15° 초과 시 거부
          {
            bool torso_exceeded = false;
            for (size_t tj = 0; tj < torso_joint_names.size(); ++tj) {
              const auto* jm = try_state->getJointModel(torso_joint_names[tj]);
              if (!jm) continue;
              double delta = std::abs(try_state->getJointPositions(jm)[0] - current_torso_vals[tj]);
              if (delta > TORSO_MAX_DELTA_RAD) {
                torso_exceeded = true;
                if (detailed_log && attempt < N_DETAILED_ATT) {
                  RCLCPP_INFO(get_logger(),
                    "    -> torso %s delta=%.1f° > ±%.0f° limit, rejecting",
                    torso_joint_names[tj].c_str(), delta * 180.0 / M_PI,
        ...
        for (const auto& s : first_collision_samples) samples_str += " [" + s + "]";
        RCLCPP_WARN(get_logger(),
          "[IK Debug] Cand %zu: found=%zu/%zu  kin_ok=%zu  collision=%zu (self=%zu env=%zu)  "
          "torso_reject=%zu  min_pos=%.4f m  min_orient=%.1f°  samples:%s",
          i + 1, n_found, N_IK_ATTEMPTS, n_kin_ok, n_collision, n_self_col, n_env_col,
          n_torso_reject, min_pos_err, min_orient_err_deg, samples_str.c_str());
